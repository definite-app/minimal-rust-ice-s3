...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Try to query the my_table directly
    println!("Contents of my_catalog.my_namespace.my_table:");
    let df = ctx.sql("SELECT * FROM my_catalog.my_namespace.my_table").await?;
    df.show().await?;

    // Try to query information_schema to get table information
    println!("\nTables in my_catalog.my_namespace:");
    let df = ctx.sql("SELECT table_name FROM information_schema.tables WHERE table_schema = 'my_namespace'").await?;
    df.show().await?;
